...
export default function Home() {
  ...
  return <main className="auth-shell"><section className="auth-card"><img className="brand-logo auth-logo" src="/logo.svg" alt="University OS"/><p className="eyebrow">UNIVERSITY OS</p><h1>Your academic workspace.</h1><p className="muted">Courses, documents, deadlines and a grounded AI study assistant in one place.</p><form onSubmit={signIn} className="stack"><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required disabled={busy}/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} required disabled={busy}/></label>{error&&<div className="error">{error}</div>}<button className="primary" disabled={busy}>{busy?"Signing in…":"Sign in"}</button></form></section></main>;
}
