...
export async function POST(req:NextRequest){
 ...
 const body=await req.json(),question=String(body.question||"").trim(),searchMode=String(body.search_mode||"auto");if(!question)return NextResponse.json({error:"Question is required."},{status:400});
 let matches:any[]=[];
 if(searchMode!=="web"&&ragUrl){
  try{
 ...
 if(!a){
  let message="Lumen generation failed.";
  try{const parsed=JSON.parse(lastDetail);const apiMessage=parsed?.error?.message;if(apiMessage)message+=": "+apiMessage;}catch{}
  if(lastStatus)message+=" (HTTP "+lastStatus+")";
  return NextResponse.json({error:message,matches},{status:502});
 }
 const answer=a?.candidates?.[0]?.content?.parts?.map((p:any)=>p.text||"").join("")||"No answer returned.";
 const grounding=a?.candidates?.[0]?.groundingMetadata;
 const webSources=(grounding?.groundingChunks??[]).map((c:any)=>c?.web).filter((x:any)=>x?.uri).map((x:any)=>({title:String(x.title||"Web source"),uri:String(x.uri)}));
 try{
 ...
 return NextResponse.json({answer,matches,webSources,searchMode});
}
