...
export async function POST(req:NextRequest){
 ...
 if(!ar.ok){const detail=await ar.text();console.error("Lumen generation failed:",detail);let message="Lumen generation failed.";try{const parsed=JSON.parse(detail);const apiMessage=parsed?.error?.message;if(apiMessage)message+=": "+apiMessage;}catch{}return NextResponse.json({error:message,matches},{status:502});}
